...
SUITE(close_tests)
{
    ...
    TEST_FIXTURE(uri_address, close_with_reason)
    {
        ...
        client.close(websocket_close_status::going_away, __U("Client disconnecting")).wait();
    }
    ...
    {
        test_websocket_server server;
        const utility::string_t close_reason = __U("Too large");
    ...
    {
        const utility::string_t close_reason = __U("Client disconnecting");
        test_websocket_server server;
