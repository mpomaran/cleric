...
SUITE(winrt_interop_tests)
{
    TEST(read_in)
    {
...

        {
            ...
            VERIFY_ARE_EQUAL(utility::string_t(value->Data()), __U("abcde"));
            value = dr->ReadString(5);
            VERIFY_ARE_EQUAL(utility::string_t(value->Data()), __U("fghij"));
        }
        {
    ...
    TEST(write_out)
    {
        ...
        auto value = ref new ::Platform::String(__U("10 4711 -10.0 hello!"));
        ...
        VERIFY_ARE_EQUAL(utility::string_t(__U("hello!")), istream.extract<utility::string_t>().get());
