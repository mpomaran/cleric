...
SUITE(ostream_tests)
{
    TEST(BasicTest1)
    {
        auto open = OPENSTR_W<uint8_t>(__U("BasicTest1.txt"));
    ...
    TEST(BasicTest2)
    {
        auto open = OPENSTR_W<uint8_t>(__U("BasicTest2.txt"));
    ...
    {
        auto open = OPENSTR_W<uint8_t>(__U("WriteSingleCharStrTest1.txt"));
    ...
    {
        auto open = OPENSTR_W<uint8_t>(__U("WriteBufferStrTest1.txt"));
        auto stream = open.get();
    ...
    {
        auto open = OPENSTR_W<uint8_t>(__U("WriteBufferAndSyncStrTest1.txt"));
        auto stream = open.get();
    ...
    TEST(tell_bug)
    {
        auto count = OPENSTR_W<uint8_t>(__U("tell_bug.txt"), std::ios_base::out | std::ios_base::trunc)
    ...
    TEST(extract_on_space)
    {
        ...
        auto open = OPENSTR_W<uint8_t>(__U("SpaceWithNumber.txt"), std::ios::trunc);
        auto stream = open.get();
        VERIFY_IS_TRUE(open.is_done());
        stream.print("  \r").wait();
        ...
        auto istream = OPENSTR_R<uint8_t>(__U("SpaceWithNumber.txt")).get();
    ...
    {
        auto open = OPENSTR_W<uint8_t>(__U("WriteFileSequential.txt"), std::ios::trunc);
        auto stream = open.get();
        ...
        stream.close().wait();
        auto istream = OPENSTR_R<uint8_t>(__U("WriteFileSequential.txt")).get();
        for (int i = 0; i < 100; i++)
        {
    ...
    {
        auto ostr = OPENSTR_W<char>(__U("implied_out_mode.txt"), std::ios::ios_base::app).get();
    ...
    {
        auto ofs = OPENSTR_W<char>(__U("file.txt")).get();
