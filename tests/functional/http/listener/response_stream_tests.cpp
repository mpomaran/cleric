...
SUITE(response_stream_tests)
{
    ...
    {
        utility::string_t fname = __U("set_response_stream_small.txt");
        fill_file(fname);
        ...
        response.headers().set_content_type(__U("text/plain; charset=utf-8"));
        ...
        p_client->next_response()
            .then([&](test_response* p_response) {
                http_asserts::assert_test_response_equals(
                    p_response, status_codes::OK, __U("text/plain; charset=utf-8"), __U("abcdefghijklmnopqrstuvwxyz"));
            })
            .wait();
    ...
    {
        utility::string_t fname = __U("set_response_stream_large.txt");
        fill_file(fname, 200);
    ...
    {
        utility::string_t fname = __U("set_response_stream_partial.txt");
        fill_file(fname, 200);
    ...
    {
        utility::string_t fname = __U("set_response_stream_chunked.txt");
        fill_file(fname, 200);
    ...
    TEST_FIXTURE(uri_address, reply_transfer_encoding_4k)
    {
        ...
        listener.support([&buf](http_request request) {
            ...
            request.reply(200, streams::istream(buf2), __U("text/plain"));
...

        {
            ...
            VERIFY_ARE_EQUAL(s.c_str(), __U("abc"));
        }
        listener.close().wait();
    }
    ...
    TEST_FIXTURE(uri_address, reply_chunked_4k, "Ignore", "Codeplex 158")
    {
        ...
        listener.support([&buf](http_request request) {
            // Ensure that it is transfer-encoded
            request.reply(200, streams::istream(buf), 4096, __U("text/plain"));
        });
