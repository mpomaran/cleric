...
SUITE(request_handler_tests)
{
    TEST_FIXTURE(uri_address, support)
    {
        ...
        listener.support(__U("CUSTOM"), [](http_request request) {
            http_asserts::assert_request_equals(request, __U("CUSTOM"), __U("/"));
        ...
        p_client->next_response()
            ...
            .wait();
        VERIFY_ARE_EQUAL(0, p_client->request(__U("CUSTOM"), __U("/")));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
            })
    ...
    TEST_FIXTURE(uri_address, exceptions_in_handler)
    {
        ...
        VERIFY_ARE_EQUAL(0, p_client->request(__U("GET"), __U("/")));
        p_client->next_response()
            .then([](test_response* p_response) {
    ...
    TEST_FIXTURE(uri_address, handle_options)
    {
        ...
        VERIFY_ARE_EQUAL(0, p_client->request(methods::OPTIONS, __U("/")));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
                VERIFY_ARE_EQUAL(__U("GET, PUT"), p_response->m_headers[__U("Allow")]);
            })
            .wait();

        // try overridding the default OPTIONS handler
        listener.support(methods::OPTIONS, [](http_request request) {
            http_asserts::assert_request_equals(request, methods::OPTIONS, __U("/"));
    ...
    TEST_FIXTURE(uri_address, handle_trace)
    {
        ...
        VERIFY_ARE_EQUAL(0, p_client->request(methods::TRCE, __U("/")));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
        ...
        listener.support(methods::TRCE, [](http_request request) {
            http_asserts::assert_request_equals(request, methods::TRCE, __U("/"));
            request.reply(status_codes::NoContent);
        });
        VERIFY_ARE_EQUAL(0, p_client->request(methods::TRCE, __U("/")));
        p_client->next_response()
    ...
    TEST_FIXTURE(uri_address, async_request_handler)
    {
        ...
        pplx::task<http_response> response =
            client.request(methods::PUT, __U("/"), buf.create_istream(), __U("text/plain"));
    ...
    {
        http_listener listener1(__U("http://localhost:45678/path1"));
        http_listener listener2(__U("http://localhost:45678/path1/path2"));
        http_listener listener3(__U("http://localhost:45678/path3"));
        listener1.open().wait();
        listener2.open().wait();
        listener3.open().wait();

        test_http_client::scoped_client client(__U("http://localhost:45678"));
        test_http_client* p_client = client.client();
        ...
        listener2.support(methods::PUT, [](http_request request) {
            http_asserts::assert_request_equals(request, methods::PUT, __U("/path4"));
            request.reply(status_codes::OK);
        });
        VERIFY_ARE_EQUAL(0, p_client->request(methods::PUT, __U("/path1/path2/path4")));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
            })
        ...
        VERIFY_ARE_EQUAL(0, p_client->request(methods::POST, __U("/path3")));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::Created);
            })
        ...
        listener1.support(methods::GET, [](http_request request) {
            http_asserts::assert_request_equals(request, methods::GET, __U("/path2/path4"));
            request.reply(status_codes::OK);
        });
        VERIFY_ARE_EQUAL(0, p_client->request(methods::GET, __U("/path1/path2/path4")));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
            })
    ...
    {
        http_listener listener1(__U("http://localhost:45679/path1"));
        http_listener listener2(__U("http://localhost:45679/path1/path2"));
        listener1.open().wait();
        listener2.open().wait();

        test_http_client::scoped_client client1(__U("http://localhost:45679"));
        test_http_client* p_client1 = client1.client();
        test_http_client::scoped_client client2(__U("http://localhost:45679"));
        ...
        VERIFY_ARE_EQUAL(0, p_client1->request(methods::GET, __U("/path1")));
        ...
        VERIFY_ARE_EQUAL(0, p_client2->request(methods::GET, __U("/path1/path2/")));
    ...
    TEST_FIXTURE(uri_address, multiple_requests)
    {
        ...
        listener.support(methods::GET, [&](http_request request) {
            http_asserts::assert_request_equals(request, methods::GET, __U("/path1"));
        ...
        VERIFY_ARE_EQUAL(0, p_client->request(methods::GET, __U("/path1")));
        VERIFY_ARE_EQUAL(0, p_client2->request(methods::GET, __U("/path1")));
        VERIFY_ARE_EQUAL(0, p_client3->request(methods::GET, __U("/path1")));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
            })
    ...
    TEST_FIXTURE(uri_address, multiple_clients_multiple_requests)
    {
        ...
        for (size_t j = 0; j < 10; ++j)
        {
            ...
            for (size_t i = 0; i < NUM_CLIENTS; ++i)
            {
                VERIFY_ARE_EQUAL(0, clients[i]->request(methods::GET, __U("/")));
    ...
    TEST_FIXTURE(uri_address, test_leaks)
    {
        ...
        listener.support(methods::PUT, [&](http_request message) {
            ...
            utility::string_t request = __U("unknown");
            auto it = message.headers().find(__U("ClientID"));
            if (it != message.headers().end())
            {
                message.reply(status_codes::OK, __U("Unknown command"));
            }
            else
            {
                message.reply(status_codes::OK, __U("ClientID missing"));
            }
        });
        ...
        for (int i = 0; i < N; ++i)
        {
            ...
            headers[__U("ClientID")] = __U("123");
            headers[__U("Request")] = __U("Upload");
            headers[__U("ImgNr")] = __U("1");
            ...
            VERIFY_ARE_EQUAL(0, p_client->request(methods::PUT, __U("/path1"), headers, data));
            p_client->next_response()
                .then([](test_response* p_response) {
                    http_asserts::assert_test_response_equals(p_response, status_codes::OK);
                })
    ...
    TEST_FIXTURE(uri_address, http_version)
    {
        ...
        http_listener listener(__U("http://localhost:45678/path1"));
        listener.open().wait();
    ...
    TEST_FIXTURE(uri_address, remote_address)
    {
        ...
        listener.support(methods::GET, [&requestCount](http_request request) {
            ...
            const string_t& localhost4 = string_t(__U("127.0.0.1"));
            const string_t& localhost6 = string_t(__U("::1"));
