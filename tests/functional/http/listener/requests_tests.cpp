...
SUITE(requests_tests)
{
    TEST_FIXTURE(uri_address, http_methods)
    {
        ...
        utility::string_t recv_methods[] = {
            __U("GET"), __U("GET"), __U("DELETE"), __U("HEAD"), __U("HEAD"), __U("POST"), __U("PUT"), __U("CUstomMETHOD")};
        ...
        for (int i = 0; i < num_methods; ++i)
        {
            ...
            VERIFY_ARE_EQUAL(0, p_client->request(send_methods[i], __U("")));
    ...
    TEST_FIXTURE(uri_address, http_body_and_body_size)
    {
        ...
        VERIFY_ARE_EQUAL(0, p_client->request(methods::GET, __U(""), data));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
            })
    ...
    TEST_FIXTURE(uri_address, large_body)
    {
        ...
        VERIFY_ARE_EQUAL(0, p_client->request(methods::GET, __U(""), __U("text/plain"), send_data));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
            })
    ...
    TEST_FIXTURE(uri_address, response_order)
    {
        ...
        listener.support([](http_request request) {
            ...
            if (str == __U("0")) tests::common::utilities::os_utilities::sleep(500);
        ...
        for (int i = 0; i < num_requests; ++i)
        {
            ...
            ss << i;
            responses.push_back(client.request(web::http::methods::PUT, __U(""), ss.str()));
        }
    ...
    TEST_FIXTURE(uri_address, uri_encoding, "Ignore", "Codeplex 201")
    {
        ...
        try
        {
            encoded_uri = uri::encode_uri(__U("/path 1/path 2")); // Path component contains encoded characters
            client.request(methods::GET, encoded_uri).wait();
            encoded_uri = uri::encode_uri(
                __U("/test?Text=J'ai besoin de trouver un personnage")); // Query string contains encoded characters
            client.request(methods::GET, encoded_uri).wait();
            encoded_uri = uri::encode_uri(__U("/path 1/path 2#fragment1")); // URI has path and fragment components
            client.request(methods::GET, encoded_uri).wait();
            encoded_uri = uri::encode_uri(
                __U("/path 1/path 2?key1=val1 val2#fragment1")); // URI has path, query and fragment components
            client.request(methods::GET, encoded_uri).wait();
        }
    ...
    TEST_FIXTURE(uri_address, https_listener, "Ignore", "Manual")
    {
        ...
        http_asserts::assert_response_equals(client.request(methods::GET, __U("")).get(), status_codes::OK);
