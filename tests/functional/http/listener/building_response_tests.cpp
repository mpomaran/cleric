...
SUITE(building_response_tests)
{
    TEST_FIXTURE(uri_address, set_body_with_content_type)
    {
        ...
        listener.support([&](http_request request) {
            ...
            response.set_body(__U("test string"), __U("text"));
        ...
        p_client->next_response()
            .then([&](test_response* p_response) {
                ...
                const ::utility::string_t expectedContentType(__U("text; charset=utf-8"));
#else
                const ::utility::string_t expectedContentType(__U("text"));
#endif
                http_asserts::assert_test_response_equals(
                    p_response, status_codes::OK, expectedContentType, __U("test string"));
            })
            .wait();
    ...
    TEST_FIXTURE(uri_address, set_body_without_content_type)
    {
        ...
        listener.support([&](http_request request) {
            http_response response(status_codes::OK);
            response.set_body(__U("test string"));
            http_asserts::assert_request_equals(request, methods::POST, __U("/"));
            request.reply(response).wait();
        });
        VERIFY_ARE_EQUAL(0u, p_client->request(methods::POST, __U("")));
        p_client->next_response()
            .then([&](test_response* p_response) {
                http_asserts::assert_test_response_equals(
                    p_response, status_codes::OK, __U("text/plain; charset=utf-8"), __U("test string"));
            })
            .wait();
    ...
    TEST_FIXTURE(uri_address, set_body_string)
    {
        ...
        listener.support([&](http_request request) {
            http_asserts::assert_request_equals(request, methods::POST, __U("/"));
            http_response response(status_codes::OK);
            utility::string_t data(__U("test data"));
        ...
        p_client->next_response()
            .then([&](test_response* p_response) {
                http_asserts::assert_test_response_equals(
                    p_response, status_codes::OK, __U("text/plain; charset=utf-8"), __U("test data"));
            })
            .wait();
    ...
    TEST_FIXTURE(uri_address, set_body_vector)
    {
        ...
        p_client->next_response()
            .then([&](test_response* p_response) {
                http_asserts::assert_test_response_equals(
                    p_response, status_codes::OK, __U("application/octet-stream"), __U("ABC"));
            })
            .wait();
