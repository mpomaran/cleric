...
{
public:
    uri_address() : m_uri(__U("http://localhost:34567/")), m_secure_uri(__U("https://localhost:8443/"))
    {
        if (!s_dummy_listener)
            s_dummy_listener =
                std::make_shared<web::http::experimental::listener::http_listener>(__U("http://localhost:30000/"));
    }
