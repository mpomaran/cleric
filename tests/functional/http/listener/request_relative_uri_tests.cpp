...
SUITE(request_relative_uri_tests)
{
    TEST_FIXTURE(uri_address, empty_base_uri)
    {
        ...
        listener.support([](http_request request) {
            VERIFY_ARE_EQUAL(__U("/path1/path2"), request.request_uri().path());
            VERIFY_ARE_EQUAL(__U("/path1/path2"), request.relative_uri().to_string());
            request.reply(status_codes::OK).wait();
        });
        VERIFY_ARE_EQUAL(0, client.request(methods::GET, __U("/path1/path2")));
    ...
    TEST_FIXTURE(uri_address, nested_paths)
    {
        ...
        listener.support([](http_request request) {
            VERIFY_ARE_EQUAL(__U("/path1/path2"), request.request_uri().path());
            VERIFY_ARE_EQUAL(__U("/path2"), request.relative_uri().to_string());
            request.reply(status_codes::OK).wait();
        });
        VERIFY_ARE_EQUAL(0, client.request(methods::GET, __U("/path1/path2")));
        client.next_response()
    ...
    TEST_FIXTURE(uri_address, nested_paths_encoding)
    {
        ...
        http_listener listener(web::http::uri_builder(m_uri).append_path(__U("/path1%20/path2%20")).to_uri());
        listener.open().wait();
        test_http_client client(m_uri);
        VERIFY_ARE_EQUAL(0, client.open());
        listener.support([](http_request request) {
            VERIFY_ARE_EQUAL(__U("/path1%20/path2%20/path3%20"), request.request_uri().path());
            VERIFY_ARE_EQUAL(__U("/path3 "), web::http::uri::decode(request.relative_uri().to_string()));
            request.reply(status_codes::OK).wait();
        });
        VERIFY_ARE_EQUAL(0, client.request(methods::GET, __U("/path1%20/path2%20/path3%20")));
        client.next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
            })
    ...
    TEST(listener_invalid_encoded_uri)
    {
        ...
        listener.support([](http_request request) { request.reply(status_codes::OK); });
        VERIFY_ARE_EQUAL(0, p_client->request(methods::GET, __U("/%invalid/uri")));
        p_client->next_response()
            .then([](test_response* p_response) {
