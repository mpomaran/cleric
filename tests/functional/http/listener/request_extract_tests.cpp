...
SUITE(request_extract_tests)
{
    TEST_FIXTURE(uri_address, extract_string)
    {
        ...
        listener.support([&](http_request request) {
            http_asserts::assert_request_equals(request, __U("PUT"), __U("/"), to_string_t(data));
            VERIFY_ARE_EQUAL(__U("text/plain"), request.headers().content_type());
            request.reply(status_codes::OK);
        });
        VERIFY_ARE_EQUAL(0, p_client->request(methods::PUT, __U(""), __U("text/plain"), data));
    ...
    TEST_FIXTURE(uri_address, extract_string_force)
    {
        ...
        VERIFY_ARE_EQUAL(0, p_client->request(methods::PUT, __U(""), __U("unknown charset"), data));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
            })
    ...
    TEST_FIXTURE(uri_address, extract_json)
    {
        ...
        listener.support([&](http_request request) {
            ...
            VERIFY_ARE_EQUAL(__U("application/json"), request.headers().content_type());
        ...
        VERIFY_ARE_EQUAL(0, p_client->request(methods::PUT, __U(""), __U("application/json"), data));
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::OK);
            })
