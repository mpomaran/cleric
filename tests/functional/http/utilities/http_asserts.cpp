...
{
    size_t index;
    while ((index = str.find_first_of(__U("#"))) != str.npos)
    {
        str.insert(index, __U("%23"));
        str.erase(index + 3, 1);
    }
    return str;
}
                                         ...
                                         const utility::string_t& relative_path)
{
    ...
    if (relative_path == __U(""))
    {
        VERIFY_ARE_EQUAL(__U("/"), request.relative_uri().to_string());
    }
    else
    {
                                              ...
                                              const utility::string_t& content_type)
{
    ...
    auto iter = p_request->m_headers.find(__U("Content-Type"));
...
static std::map<utility::string_t, utility::string_t> parse_headers(utility::istringstream_t& ss)
{
    ...
    {
        const size_t colon_index = header_line.find(__U(":"));
                                               ...
                                               const utility::string_t& content_type)
{
    ...
    p_response->match_header(__U("Content-Type"), found_content);
