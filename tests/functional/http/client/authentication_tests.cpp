...
SUITE(authentication_tests)
{
    TEST_FIXTURE(uri_address, auth_no_data, "Ignore:Linux", "89", "Ignore:Apple", "89")
    {
        pplx::task<void> t, t2;
        {
            ...
            web::credentials cred(__U("some_user"), __U("some_password")); // WinHTTP requires non-empty password
            ...
            t = scoped.server()->next_request().then([&](test_request* p_request) {
                ...
                headers[__U("WWW-Authenticate")] = __U("Basic realm = \"WallyWorld\"");

                // unauthorized
                p_request->reply(status_codes::Unauthorized, __U("Authentication Failed"), headers);
            });
...
// TFS 648783
    ...
    TEST_FIXTURE(uri_address, proxy_auth_known_contentlength, "Ignore:Linux", "88", "Ignore:Apple", "88")
    {
        pplx::task<void> t, t2;
        {
            ...
            http_client client(m_uri, client_config);
            const method mtd = methods::POST;
            utility::string_t contents(__U("Hello World"));
            ...
            t2 = scoped.server()->next_request().then([&](test_request* p_request) {
                http_asserts::assert_test_request_equals(
                    p_request, methods::POST, __U("/"), __U("text/plain; charset=utf-8"), contents);
    ...
    {
        web::http::uri uri(__U("http://localhost:34567/"));
...
// Must specify content length with winrt client, so this test case isn't possible.
    ...
    TEST_FIXTURE(uri_address, proxy_auth_unknown_contentlength, "Ignore:Linux", "88", "Ignore:Apple", "88")
    {
        pplx::task<void> t;
        {
            ...
            auto replyFunc = [&](test_request* p_request) {
                utility::string_t contents(__U("a"));
                http_asserts::assert_test_request_equals(
                    p_request, methods::POST, __U("/"), __U("application/octet-stream"), contents);
    ...
    TEST_FIXTURE(uri_address, empty_username_password)
    {
        ...
        auto t = scoped.server()->next_request().then([&](test_request* p_request) {
            std::map<utility::string_t, utility::string_t> headers;
            headers[__U("h1")] = __U("data1");
            // Auth header
            headers[__U("WWW-Authenticate")] = __U("Basic realm = \"myRealm\"");
            // unauthorized
            p_request->reply(status_codes::Unauthorized, __U("Authentication Failed"), headers, "a");
        });

        http_response response = client.request(methods::GET).get();
        auto str_body = response.extract_vector().get();
        auto h1 = response.headers()[__U("h1")];
        VERIFY_ARE_EQUAL(status_codes::Unauthorized, response.status_code());
        VERIFY_ARE_EQUAL(str_body[0], 'a');
        VERIFY_ARE_EQUAL(h1, __U("data1"));
        t.get();
    }
#endif
    ...
    TEST_FIXTURE(uri_address, error_after_valid_credentials, "Ignore:Linux", "89", "Ignore:Apple", "89")
    {
        pplx::task<void> t;
        {
            web::http::uri uri(__U("http://localhost:34569/"));
            test_http_server::scoped_server scoped(uri);
            http_client_config client_config;
            web::credentials cred(__U("some_user"), __U("some_password"));
            client_config.set_credentials(cred);
            ...
            auto replyFunc = [&](test_request* p_request) {
                ...
                // Auth header
                headers[__U("WWW-Authenticate")] = __U("Basic realm = \"WallyWorld\"");
                headers[__U("h1")] = __U("data2");
                // still unauthorized after the user has resent the request with the credentials
                p_request->reply(status_codes::Unauthorized, __U("Authentication Failed"), headers, "def");
            };
            ...
            t = scoped.server()
                    ...
                    .then([&](test_request* p_request) {
                        ...
                        headers[__U("WWW-Authenticate")] = __U("Basic realm = \"myRealm\"");
                        // unauthorized
                        p_request->reply(status_codes::Unauthorized, __U("Authentication Failed"), headers, "abc");
                    })
                    .then([&scoped, &replyFunc]() {
                        // Client resent the request
                        return scoped.server()->next_request().then(replyFunc);
    ...
    TEST_FIXTURE(server_properties, basic_anonymous_auth_with_basic_cred, "Requires", "Server;UserName;Password")
    {
        ...
        req.headers().add(__U("UserName"), m_username);
        req.headers().add(__U("Password"), m_password);
    ...
    TEST_FIXTURE(uri_address, auth_producer_consumer_buffer)
    {
        ...
        config.set_credentials(web::credentials(__U("USERNAME"), __U("PASSWORD")));
        ...
        t = scoped.server()->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(
                p_request, methods::POST, __U("/"), __U("application/octet-stream"), __U("aaaa"));
            std::map<utility::string_t, utility::string_t> headers;
            headers[__U("WWW-Authenticate")] = __U("Basic realm = \"WallyWorld\"");
    ...
    TEST_FIXTURE(uri_address, auth_producer_comsumer_buffer_fail)
    {
        ...
        pplx::task<void> t;
        {
            ...
            auto replyFunc = [&](test_request* p_request) {
                http_asserts::assert_test_request_equals(
                    p_request, methods::POST, __U("/"), __U("application/octet-stream"), __U("a"));
                std::map<utility::string_t, utility::string_t> headers;
                headers[__U("WWW-Authenticate")] = __U("Basic realm = \"WallyWorld2\"");
    ...
    TEST_FIXTURE(uri_address, failed_authentication_attempt, "Ignore:Linux", "89", "Ignore:Apple", "89")
    {
        handle_timeout([] {
            http_client_config config;
            web::credentials cred(__U("user"), __U("schmuser"));
            config.set_credentials(cred);
            http_client client(__U("https://apis.live.net"), config);
            http_response response = client.request(methods::GET, __U("V5.0/me/skydrive/files")).get();
            VERIFY_ARE_EQUAL(status_codes::Unauthorized, response.status_code());
    ...
    void auth_test_impl(bool fail)
    {
        ...
        client_config.set_credentials(cred);
        http_client client(__U("http://httpbin.org/basic-auth/user1/user1"), client_config);
