...
SUITE(building_request_tests)
{
    TEST_FIXTURE(uri_address, simple_values)
    {
        ...
        // Set a path once.
        const utility::string_t custom_path1 = __U("/hey/custom/path");
        ...
        msg.set_request_uri(custom_path1);
        VERIFY_ARE_EQUAL(custom_path1, msg.relative_uri().to_string());
        const utility::string_t custom_path2 = __U("/yes/you/there");
    ...
    TEST_FIXTURE(uri_address, body_types)
    {
        ...
        // Body data types.
        const method method(__U("CUSTOMmethod"));
        utility::string_t str_body(__U("YES_BASIC_STRING BODY"));
        ...
        utility::string_t custom_content = __U("YESNOW!");

        // vector - no content type.
        http_request msg(method);
        msg.set_body(std::move(vector_move_body));
        VERIFY_ARE_EQUAL(__U("application/octet-stream"), msg.headers()[__U("Content-Type")]);
        ...
        msg.headers().add(__U("Content-Type"), custom_content);
        msg.set_body(std::move(vector_move_body));
        VERIFY_ARE_EQUAL(custom_content, msg.headers()[__U("Content-Type")]);
        p_server->next_request().then([&](test_request* p_request) {
            auto received = p_request->m_body;
            auto sent = vector_body;
            VERIFY_IS_TRUE(received == sent);
        ...
        VERIFY_ARE_EQUAL(__U("text/plain; charset=utf-8"), msg.headers()[__U("Content-Type")]);
        p_server->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(
                p_request, method, __U("/"), __U("text/plain; charset=utf-8"), str_body);
            p_request->reply(200);
        });
        http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);
        ...
        VERIFY_ARE_EQUAL(custom_content, msg.headers()[__U("Content-Type")]);
        p_server->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, method, __U("/"), custom_content, str_body);
            p_request->reply(200);
        });
        http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);
    }
    ...
    TEST_FIXTURE(uri_address, empty_bodies)
    {
        ...
        p_server->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, method, __U("/"), __U("application/octet-stream"));
        ...
        p_server->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, method, __U("/"), __U("text/plain; charset=utf-8"));
            VERIFY_ARE_EQUAL(0u, p_request->m_body.size());
            p_request->reply(200);
        });
        http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);
    ...
    TEST_FIXTURE(uri_address, set_body)
    {
        ...
        utility::string_t data(__U("YOU KNOW~!!!!!"));
        utility::string_t content_type = __U("text/plain; charset=utf-8");

        // without content type
        http_request msg(mtd);
        msg.set_body(data);
        VERIFY_ARE_EQUAL(content_type, msg.headers()[__U("Content-Type")]);
        scoped.server()->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, mtd, __U("/"), content_type, data);
            p_request->reply(200);
        });
        http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);

        // with content type
        content_type = __U("YESYES");
#ifdef _UTF16_STRINGS
        const utility::string_t expected_content_type = __U("YESYES; charset=utf-8");
#else
        const utility::string_t expected_content_type = __U("YESYES");
#endif
        msg = http_request(mtd);
        msg.set_body(data, content_type);
        VERIFY_ARE_EQUAL(expected_content_type, msg.headers()[__U("Content-Type")]);
        scoped.server()->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, mtd, __U("/"), expected_content_type, data);
            p_request->reply(200);
        });
        http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);
    }
    ...
    TEST_FIXTURE(uri_address, set_content_length_locale, "Ignore:Android", "Locale unsupported on Android")
    {
        ...
        VERIFY_ARE_EQUAL(__U("1000"), req.headers()[web::http::header_names::content_length]); // fr_RF would have 1 000
    }
    ...
    TEST_FIXTURE(uri_address, set_port_locale, "Ignore:Android", "Locale unsupported on Android")
    {
        ...
        utility::string_t data(__U("STRING data 1000"));
        t = scoped.server()->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(
                p_request, methods::PUT, __U("/"), __U("text/plain; charset=utf-8"), data);
            p_request->reply(200);
        });
