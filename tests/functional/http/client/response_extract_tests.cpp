...
SUITE(response_extract_tests)
{
                                               ...
                                               const std::basic_string<CharType>& data)
    {
        ...
        p_server->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, method, __U("/"));
            VERIFY_ARE_EQUAL(0u, p_request->reply(code, __U(""), headers, data));
        });
    ...
    TEST_FIXTURE(uri_address, extract_string)
    {
        ...
        http_response rsp = send_request_response(scoped.server(), &client, __U("text/plain"), data);
        VERIFY_ARE_EQUAL(to_string_t(data), rsp.extract_string().get());

        // us-ascii
        rsp = send_request_response(scoped.server(), &client, __U("text/plain;  charset=  us-AscIi"), data);
        VERIFY_ARE_EQUAL(to_string_t(data), rsp.extract_string().get());

        // Latin1
        rsp = send_request_response(scoped.server(), &client, __U("text/plain;charset=iso-8859-1"), data);
        VERIFY_ARE_EQUAL(to_string_t(data), rsp.extract_string().get());

        // utf-8
        rsp = send_request_response(scoped.server(), &client, __U("text/plain; charset  =  UTF-8"), data);
        VERIFY_ARE_EQUAL(to_string_t(data), rsp.extract_string().get());
        ...
        // no content length
        rsp = send_request_response(scoped.server(), &client, __U(""), utility::string_t());
        auto str = rsp.to_string();
        // If there is no Content-Type in the response, make sure it won't throw when we ask for string
        if (str.find(__U("Content-Type")) == std::string::npos)
        {
            VERIFY_ARE_EQUAL(utility::string_t(__U("")), rsp.extract_string().get());
        }

        // utf-16le
        data = "YES NOW, HERHEHE****";
        utf16string wdata(utf8_to_utf16(data));
        rsp = send_request_response(scoped.server(), &client, __U("text/plain; charset=utf-16le"), wdata);
        VERIFY_ARE_EQUAL(to_string_t(data), rsp.extract_string().get());

        // utf-16be
        wdata = switch_endian_ness(wdata);
        rsp = send_request_response(scoped.server(), &client, __U("text/plain; charset=utf-16be"), wdata);
        VERIFY_ARE_EQUAL(to_string_t(data), rsp.extract_string().get());

        // utf-16 no BOM (utf-16be)
        rsp = send_request_response(scoped.server(), &client, __U("text/plain; charset=utf-16"), wdata);
        VERIFY_ARE_EQUAL(to_string_t(data), rsp.extract_string().get());
    ...
    TEST_FIXTURE(uri_address, extract_utf16string)
    {
        ...
        rsp = send_request_response(scoped.server(), &client, __U("text/plain; charset=utf-16be"), wdatabe);
        VERIFY_ARE_EQUAL(wdata, rsp.extract_utf16string().get());

        // utf-16 no BOM (utf-16be)
        rsp = send_request_response(scoped.server(), &client, __U("text/plain; charset=utf-16"), wdatabe);
        VERIFY_ARE_EQUAL(wdata, rsp.extract_utf16string().get());
        ...
        start[0] = 0xFF;
        start[1] = 0xFE;
        rsp = send_request_response(scoped.server(), &client, __U("text/plain; charset=utf-16"), wdatale);
        VERIFY_ARE_EQUAL(wdata, rsp.extract_utf16string().get());
    }
    ...
    TEST_FIXTURE(uri_address, extract_string_force)
    {
        ...
        std::string data("YOU KNOW ITITITITI");
        http_response rsp = send_request_response(scoped.server(), &client, __U("bad unknown charset"), data);
        VERIFY_ARE_EQUAL(to_string_t(data), rsp.extract_string(true).get());
        rsp = send_request_response(scoped.server(), &client, __U("bad unknown charset"), data);
    ...
    TEST_FIXTURE(uri_address, extract_string_incorrect)
    {
        ...
        http_response rsp = send_request_response(scoped.server(), &client, __U("non_text"), data);
        VERIFY_THROWS(rsp.extract_string().get(), http_exception);

        // with unknown charset
        rsp = send_request_response(scoped.server(), &client, __U("text/plain; charset=uis-ascii"), data);
        VERIFY_THROWS(rsp.extract_string().get(), http_exception);
    }
    ...
    TEST_FIXTURE(uri_address, extract_empty_string)
    {
        ...
        listener.support([](http_request msg) {
            ...
            response.set_body(ResponseStreamBuf.create_istream(), __U("text/plain"));
    ...
    TEST_FIXTURE(uri_address, extract_json)
    {
        ...
        // default encoding (Latin1)
        json::value data = json::value::string(__U("JSON string object"));
        http_response rsp =
            send_request_response(scoped.server(), &client, __U("application/json"), to_utf8string(data.serialize()));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());

        // us-ascii
        rsp = send_request_response(
            scoped.server(), &client, __U("application/json;  charset=  us-AscIi"), to_utf8string(data.serialize()));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());

        // Latin1
        rsp = send_request_response(
            scoped.server(), &client, __U("application/json;charset=iso-8859-1"), to_utf8string(data.serialize()));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());

        // utf-8
        rsp = send_request_response(
            scoped.server(), &client, __U("application/json; charset  =  UTF-8"), to_utf8string((data.serialize())));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());
        ...
        if (str.find(__U("Content-Type")) == std::string::npos)
        {
            VERIFY_ARE_EQUAL(utility::string_t(__U("null")), rsp.extract_json().get().serialize());
        }

#ifdef _WIN32
        // utf-16le
        auto utf16str = data.serialize();
        rsp = send_request_response(scoped.server(), &client, __U("application/json; charset=utf-16le"), utf16str);
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());

        // utf-16be
        utf16string modified_data = data.serialize();
        modified_data = switch_endian_ness(modified_data);
        rsp = send_request_response(scoped.server(), &client, __U("application/json; charset=utf-16be"), modified_data);
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());

        // utf-16 no BOM (utf-16be)
        rsp = send_request_response(scoped.server(), &client, __U("application/json; charset=utf-16"), modified_data);
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());

        // utf-16 big endian BOM.
        modified_data.insert(modified_data.begin(), __U('\0'));
        ...
        rsp = send_request_response(scoped.server(), &client, __U("text/json"), to_utf8string(data.serialize()));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());
        rsp = send_request_response(scoped.server(), &client, __U("text/x-json"), to_utf8string(data.serialize()));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());
        rsp = send_request_response(scoped.server(), &client, __U("text/javascript"), to_utf8string(data.serialize()));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());
        rsp = send_request_response(scoped.server(), &client, __U("text/x-javascript"), to_utf8string(data.serialize()));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());
        rsp = send_request_response(
            scoped.server(), &client, __U("application/javascript"), to_utf8string(data.serialize()));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());
        rsp = send_request_response(
            scoped.server(), &client, __U("application/x-javascript"), to_utf8string(data.serialize()));
        VERIFY_ARE_EQUAL(data.serialize(), rsp.extract_json().get().serialize());
    }
    ...
    TEST_FIXTURE(uri_address, extract_json_force)
    {
        ...
        json::value data = json::value::string(__U("JSON string object"));
        http_response rsp =
            send_request_response(scoped.server(), &client, __U("bad charset"), to_utf8string(data.serialize()));
    ...
    TEST_FIXTURE(uri_address, extract_json_incorrect)
    {
        ...
        // with non matching content type.
        json::value json_data = json::value::string(__U("JSON string object"));
        http_response rsp = send_request_response(scoped.server(), &client, __U("bad guy"), json_data.serialize());
        VERIFY_THROWS(rsp.extract_json().get(), http_exception);

        // with unknown charset.
        rsp = send_request_response(
            scoped.server(), &client, __U("application/json; charset=us-askjhcii"), json_data.serialize());
        VERIFY_THROWS(rsp.extract_json().get(), http_exception);
    }
    ...
    TEST_FIXTURE(uri_address, set_stream_try_extract_json)
    {
        ...
        scoped.server()->next_request().then([](test_request* req) {
            std::map<utility::string_t, utility::string_t> headers;
            headers[header_names::content_type] = __U("application/json");
            req->reply(status_codes::OK, __U("OK"), headers, __U("{true}"));
        });
    ...
    TEST_FIXTURE(uri_address, extract_vector)
    {
        ...
        http_response rsp = send_request_response(scoped.server(), &client, __U("text/plain; charset=unknown"), data);
        ...
        rsp = send_request_response(scoped.server(), &client, __U("text/plain;  charset=utf-16LE"), data);
        VERIFY_ARE_EQUAL(vector_data, rsp.extract_vector().get());

        // textual type with utf-16be
        rsp = send_request_response(scoped.server(), &client, __U("text/plain;  charset=UTF-16be"), data);
        VERIFY_ARE_EQUAL(vector_data, rsp.extract_vector().get());

        // textual type with utf-16
        rsp = send_request_response(scoped.server(), &client, __U("text/plain;  charset=utf-16"), data);
        VERIFY_ARE_EQUAL(vector_data, rsp.extract_vector().get());

        // non textual content type
        rsp = send_request_response(scoped.server(), &client, __U("blah;  charset=utf-16"), data);
        VERIFY_ARE_EQUAL(vector_data, rsp.extract_vector().get());
    }
    ...
    TEST_FIXTURE(uri_address, set_stream_try_extract_vector)
    {
        ...
        scoped.server()->next_request().then([](test_request* req) {
            std::map<utility::string_t, utility::string_t> headers;
            headers[header_names::content_type] = __U("text/plain");
            req->reply(status_codes::OK, __U("OK"), headers, __U("data"));
        });
    ...
    TEST_FIXTURE(uri_address, head_response)
    {
        ...
        headers[__U("Content-Length")] = __U("100");
        scoped.server()->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, method, __U("/"));
            VERIFY_ARE_EQUAL(0u, p_request->reply(code, __U(""), headers));
        });
