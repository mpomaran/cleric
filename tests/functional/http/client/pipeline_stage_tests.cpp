...
SUITE(pipeline_stage_tests)
{
    TEST_FIXTURE(uri_address, http_counting_methods)
    {
        ...
        utility::string_t send_methods[] = {methods::GET,
                                            ...
                                            methods::PATCH,
                                            __U("CUstomMETHOD")};
        utility::string_t recv_methods[] = {__U("GET"),
                                            __U("GET"),
                                            __U("DELETE"),
                                            __U("HEAD"),
#ifdef _WIN32
        ...
        for (int i = 0; i < num_methods; ++i)
        {
            p_server->next_request().then([i, &recv_methods](test_request* p_request) {
                http_asserts::assert_test_request_equals(p_request, recv_methods[i], __U("/"));
    ...
    public:
        ...
        {
            request.headers().set_content_type(__U("modified content type"));
    ...
    TEST_FIXTURE(uri_address, pipeline_stage_inspect_response)
    {
        ...
        scoped.server()->next_request().then([](test_request* request) {
            http_asserts::assert_test_request_equals(request, methods::GET, __U("/"), __U("modified content type"));
        ...
        VERIFY_ARE_EQUAL(__U("1, 2"), response.headers()[__U("My Header")]);
    }
