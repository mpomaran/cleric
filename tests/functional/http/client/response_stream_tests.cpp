...
SUITE(response_stream_tests)
{
    TEST_FIXTURE(uri_address, set_response_stream_producer_consumer_buffer)
    {
        ...
        p_server->next_request().then([&](test_request* p_request) {
            ...
            p_request->reply(200, __U(""), headers, "This is just a bit of a string");
        });
    ...
    TEST_FIXTURE(uri_address, response_stream_file_stream)
    {
        ...
        p_server->next_request().then([&](test_request* p_request) {
            std::map<utility::string_t, utility::string_t> headers;
            headers[__U("Content-Type")] = __U("text/plain");
            p_request->reply(200, __U(""), headers, message);
        });

        {
            auto fstream = OPENSTR_W<uint8_t>(__U("response_stream.txt")).get();
            ...
            streams::basic_istream<uint8_t> fistream = OPENSTR_R<uint8_t>(__U("response_stream.txt")).get();
    ...
    TEST_FIXTURE(uri_address, response_stream_file_stream_close_early)
    {
        ...
        p_server->next_request().then([&](test_request* p_request) {
            std::map<utility::string_t, utility::string_t> headers;
            headers[__U("Content-Type")] = __U("text/plain");
            p_request->reply(200, __U(""), headers, "A world without string is chaos.");
        });
#endif

        auto fstream = OPENSTR_W<uint8_t>(__U("response_stream_file_stream_close_early.txt")).get();
    ...
    TEST_FIXTURE(uri_address, response_stream_large_file_stream)
    {
        ...
        utility::string_t fname(__U("response_stream_large_file_stream.txt"));
        ...
        p_server->next_request().then([&](test_request* p_request) {
            ...
            p_request->reply(200, __U(""), headers, responseData);
        });
    ...
    TEST_FIXTURE(uri_address, content_ready)
    {
        ...
        listener.support([responseData](http_request request) {
            streams::producer_consumer_buffer<uint8_t> buf;
            http_response response(200);
            response.set_body(buf.create_istream(), __U("text/plain"));
            response.headers().add(header_names::connection, __U("close"));
    ...
    {
        http_client client(m_uri);
        utility::string_t responseData(__U("Hello world"));
        ...
        listener.support([responseData](http_request request) {
            ...
            response.headers().add(header_names::transfer_encoding, __U("chunked"));
    ...
    TEST_FIXTURE(uri_address, xfer_chunked_multiple_chunks)
    {
        ...
        {
            utility::string_t fname(__U("xfer_chunked_multiple_chunks.txt"));
            auto fstream = OPENSTR_W<uint8_t>(fname).get();
