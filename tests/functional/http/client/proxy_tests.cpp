...
static const auto proxy_uri = __U("http://netproxy.redmond.corp.microsoft.com");
...
SUITE(proxy_tests)
{
    ...
    TEST_FIXTURE(uri_address, auto_discovery_proxy)
    {
        ...
        auto t = scoped.server()->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(
                p_request, methods::PUT, __U("/"), __U("text/plain"), __U("this is a test"));
        ...
        http_asserts::assert_response_equals(client.request(methods::PUT, __U("/"), __U("this is a test")).get(),
                                             status_codes::OK);
    ...
    TEST_FIXTURE(uri_address, disabled_proxy)
    {
        ...
        auto t = scoped.server()->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(
                p_request, methods::PUT, __U("/"), __U("text/plain"), __U("sample data"));
            p_request->reply(status_codes::OK);
        });
        ...
        http_client client(m_uri, config);
        http_asserts::assert_response_equals(client.request(methods::PUT, __U("/"), __U("sample data")).get(),
                                             status_codes::OK);
    ...
    TEST_FIXTURE(uri_address, no_proxy_options_on_winrt)
    {
        ...
        VERIFY_THROWS(client.request(methods::GET, __U("/")).get(), http_exception);
    }
#endif
                 ...
                 "Manual")
    {
        ...
        web::credentials cred(__U("artur"), __U("fred")); // relax, this is not my real password
        ...
        http_client client(__U("http://www.microsoft.com"), config);
    ...
    TEST_FIXTURE(uri_address, http_proxy, "Ignore", "Manual")
    {
        ...
        http_client client(__U("http://httpbin.org"), config);
    ...
    TEST_FIXTURE(uri_address, https_proxy, "Ignore", "Manual")
    {
        ...
        http_client client(__U("https://httpbin.org"), config);
