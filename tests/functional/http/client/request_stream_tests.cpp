...
SUITE(request_stream_tests)
{
    ...
    TEST_FIXTURE(uri_address, ixhr2_transfer_encoding)
    {
        ...
        VERIFY_THROWS(client.request(methods::POST, __U(""), buf.create_istream(), 1).get(), http_exception);
    }
#endif

    TEST_FIXTURE(uri_address, set_body_stream_1)
    {
        utility::string_t fname = __U("set_body_stream_1.txt");
        fill_file(fname);
        ...
        p_server->next_request().then([&](test_request* p_request) {
            ...
            VERIFY_ARE_EQUAL(__U("abcdefghijklmnopqrstuvwxyz"), ::utility::conversions::to_string_t(str_body));
            p_request->reply(200);
        });
    ...
    {
        utility::string_t fname = __U("set_body_stream_2.txt");
        fill_file(fname);
        ...
        const uri& address, bool withContentLength, size_t chunksize, utility::string_t fname)
    {
        ...
        if (withContentLength)
        {
            http_asserts::assert_response_equals(
                client.request(methods::POST, __U(""), stream, 26, __U("text/plain")).get(), status_codes::OK);
        }
        else
        {
#if defined __cplusplus_winrt
            http_asserts::assert_response_equals(
                client.request(methods::POST, __U(""), stream, 26, __U("text/plain")).get(), status_codes::OK);
#else
            http_asserts::assert_response_equals(client.request(methods::POST, __U(""), stream, __U("text/plain")).get(),
                                                 status_codes::OK);
#endif
        }
    ...
    {
        stream_request_impl(m_uri, false, 64 * 1024, __U("without_content_length_1.txt"));
    }

    TEST_FIXTURE(uri_address, without_content_length_2)
    {
        stream_request_impl(m_uri, false, 1024, __U("without_content_length_2.txt"));
    }
#endif

    TEST_FIXTURE(uri_address, with_content_length_1)
    {
        stream_request_impl(m_uri, true, 64 * 1024, __U("with_content_length_1.txt"));
    }
    ...
    {
        utility::string_t fname = __U("stream_partial_from_start.txt");
        fill_file(fname, 200);
    ...
    {
        utility::string_t fname = __U("stream_partial_from_middle.txt");
        fill_file(fname, 100);
...
// specify a Content-Length.
    ...
    TEST_FIXTURE(uri_address, stream_close_early)
    {
        ...
        // Make request.
        streams::producer_consumer_buffer<uint8_t> buf;
        auto responseTask = client.request(methods::PUT, __U(""), buf.create_istream());
    ...
    TEST_FIXTURE(uri_address, stream_close_early_with_exception_and_contentlength)
    {
        ...
        // Make request.
        streams::producer_consumer_buffer<uint8_t> buf;
        auto responseTask = client.request(methods::PUT, __U(""), buf.create_istream(), 10);
