...
SUITE(iterator_tests)
{
    ...
    TEST(non_composites_member_preincrement)
    {
        ...
        validate_array_and_object_throw(json::value::string(__U("Hello!")));
    }
    ...
    TEST(objects_constructed)
    {
        json::value val1;
        val1[__U("a")] = 44;
        val1[__U("b")] = json::value(true);
        val1[__U("c")] = json::value(false);
        ...
        for (auto iter = std::begin(val1.as_object()); iter != std::end(val1.as_object()); ++iter)
        {
            ...
            {
                case 0:
                    VERIFY_ARE_EQUAL(__U("a"), key);
                    VERIFY_IS_TRUE(value.is_number());
                    break;
                case 1:
                    VERIFY_ARE_EQUAL(__U("b"), key);
                    VERIFY_IS_TRUE(value.is_boolean());
                    break;
                case 2:
                    VERIFY_ARE_EQUAL(__U("c"), key);
                    VERIFY_IS_TRUE(value.is_boolean());
                    break;
            }
            count++;
    ...
    TEST(objects_parsed)
    {
        json::value val1 = json::value::parse(__U("{\"a\": 44, \"b\": true, \"c\": false}"));
    ...
    TEST(arrays_parsed)
    {
        json::value val1 = json::value::parse(__U("[44, true, false]"));
    ...
    {
        {
            // for_each
            size_t count = 0;
            json::value v_array = json::value::parse(__U("[44, true, false]"));
