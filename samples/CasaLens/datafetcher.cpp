...
pplx::task<json::value> CasaLens::get_events(const utility::string_t& postal_code)
{
    ...
    return ev_client.request(methods::GET, event_url)
        ...
        .then([](json::value event_json) {
            ...
            if (!event_json[__U("events")][__U("event")].is_null())
            {
                event_result_node[__U("events")] = json::value::array();

                int i = 0;
                for (auto& iter : event_json[__U("events")][__U("event")].as_array())
                {
                    const auto& event = iter;
                    auto iTitle = event.as_object().find(__U("title"));
                    if (iTitle == event.as_object().end())
                    {
                        throw web::json::json_exception(__U("title key not found"));
                    }
                    event_result_node[events_json_key][i][__U("title")] = iTitle->second;
                    auto iUrl = event.as_object().find(__U("url"));
                    if (iUrl == event.as_object().end())
                    {
                        throw web::json::json_exception(__U("url key not found"));
                    }
                    event_result_node[events_json_key][i][__U("url")] = iUrl->second;
                    auto iStartTime = event.as_object().find(__U("start_time"));
                    if (iStartTime == event.as_object().end())
                    {
                        throw web::json::json_exception(__U("start_time key not found"));
                    }
                    event_result_node[events_json_key][i][__U("starttime")] = iStartTime->second;
                    auto iDescription = event.as_object().find(__U("description"));
                    if (iDescription == event.as_object().end())
                    {
                        throw web::json::json_exception(__U("descriotion key not found"));
                    }
                    event_result_node[events_json_key][i][__U("description")] = iDescription->second;
                    auto iVenueAddress = event.as_object().find(__U("venue_address"));
                    if (iVenueAddress == event.as_object().end())
                    {
                        throw web::json::json_exception(__U("venue_address key not found"));
                    }
                    auto iCityName = event.as_object().find(__U("city_name"));
                    if (iCityName == event.as_object().end())
                    {
                        throw web::json::json_exception(__U("city_name key not found"));
                    }
                    event_result_node[events_json_key][i][__U("venue_address")] =
                        json::value::string(iVenueAddress->second.as_string() + __U(" ") + iCityName->second.as_string());
            ...
            else
            {
                ...
                event_result_node[events_json_key][error_json_key] = event_json[__U("events")][__U("description")];
            }
...
pplx::task<json::value> CasaLens::get_weather(const utility::string_t& postal_code)
{
    ...
    ub_weather.append_query(__U("units"), __U("imperial"));
    ...
    return weather_client.request(methods::GET)
        ...
        .then([](utility::string_t weather_str) {
            ...
            auto& j = weather_json[__U("list")][0][__U("main")];

            json::value weather_result_node = json::value::object();
            weather_result_node[weather_json_key] = json::value::object();
            auto& w = weather_result_node[__U("weather")];
            w[__U("temperature")] = j[__U("temp")];
            w[__U("pressure")] = j[__U("pressure")];
            w[__U("temp_min")] = j[__U("temp_min")];
            w[__U("temp_max")] = j[__U("temp_max")];
            w[__U("image")] =
                json::value::string(__U("http://openweathermap.org/img/w/") +
                                    weather_json[__U("list")][0][__U("weather")][0][__U("icon")].as_string() + __U(".png"));
            w[__U("description")] = weather_json[__U("list")][0][__U("weather")][0][__U("description")];
...
pplx::task<json::value> CasaLens::get_pictures(const utility::string_t& location, const utility::string_t& count)
{
    ...
    ub_bing.append_query(__U("Query"), __U("'") + location + __U("'"));
    ub_bing.append_query(__U("$top"), count);
    ub_bing.append_query(__U("ImageFilters"), __U("'Size:Medium'"));
    ...
    return bing_client.request(methods::GET)
        ...
        .then([](json::value image_json) {
            ...
            int i = 0;
            for (auto& val : image_json[__U("d")][__U("results")].as_object())
            {
                const json::object& image = val.second.as_object();
                auto iMediaUrl = image.find(__U("MediaUrl"));
                if (iMediaUrl == image.end())
                {
                    throw web::json::json_exception(__U("MediaUrl key not found"));
                }
...
pplx::task<json::value> CasaLens::get_movies(const utility::string_t& postal_code)
{
    ...
    ub_movie.append_query(__U("startDate"), get_date());
    ub_movie.append_query(__U("zip"), postal_code);
    ub_movie.append_query(casalens_creds::movies_keyname, casalens_creds::movies_key);
    ub_movie.append_query(__U("imageSize"), __U("Sm"));
    ...
    return tms_client.request(methods::GET)
        ...
        .then([](json::value movie_json) {
            ...
            if (movie_json.size())
            {
                ...
                for (auto iter = movie_json.as_array().cbegin(); iter != movie_json.as_array().cend() && i < num_movies;
                     iter++, i++)
                {
                    auto iShowTimes = iter->as_object().find(__U("showtimes"));
                    if (iShowTimes == iter->as_object().end())
                    {
                        throw web::json::json_exception(__U("showtimes key not found"));
                    }
                    ...
                    auto iTitle = iter->as_object().find(__U("title"));
                    if (iTitle == iter->as_object().end())
                    {
                        throw web::json::json_exception(__U("title key not found"));
                    }
                    temp[i][__U("title")] = iTitle->second;
                    for (auto iter2 = showtimes.as_array().cbegin();
                         iter2 != showtimes.as_array().cend() && j < num_movies;
                         iter2++, j++)
                    {
                        auto iTheatre = iter2->as_object().find(__U("theatre"));
                        if (iTheatre == iter2->as_object().end())
                        {
                            throw web::json::json_exception(__U("theatre key not found"));
                        }
                        auto iName = iTheatre->second.as_object().find(__U("name"));
                        ...
                        {
                            temp[i][__U("theatre")][++theater_index][__U("name")] =
                                json::value::string(theater); // Add theater entry
                            showtime_index = 0;
                            current_theater = theater;
                        }
                        auto iDateTime = iter2->as_object().find(__U("dateTime"));
                        if (iDateTime == iter2->as_object().end())
                        {
                            throw web::json::json_exception(__U("dateTime key not found"));
                        }
                        temp[i][__U("theatre")][theater_index][__U("datetime")][showtime_index++] =
            ...
            else
            {
                ...
                movie_result_node[movies_json_key][error_json_key] =
                    json::value::string(__U("Failed to fetch movie data"));
            }
        ...
        .then([=](json::value movie_result) {
            try
            {
                ...
                auto date = get_date();
                std::wstring year = date.substr(0, date.find(__U("-")));

                for (auto& iter : movie_result[movies_json_key].as_object())
                {
                    auto title = iter.second[__U("title")].as_string();
                    auto searchStr = title + __U(" ") + year + __U(" new movie poster");
                    poster_tasks.push_back(get_pictures(searchStr, __U("1")));
                ...
                for (unsigned int i = 0; i < poster_tasks.size(); i++)
                {
                    ...
                    movie_result[movies_json_key][i][__U("poster")] = poster_url;
                }
            }
            catch (...)
            {
...
void CasaLens::fetch_data(http_request message, const std::wstring& postal_code, const std::wstring& location)
{
    ...
    try
    {
        ...
        if (resp_data.is_null())
        {
            ...
            tasks.push_back(get_pictures(location, __U("4")));
...
void CasaLens::get_data(http_request message, const std::wstring& input_text)
{
    if (!is_number(utility::conversions::to_utf8string(input_text)))
    {
        ...
        maps_builder.append_query(__U("locality"), input_text);
        ...
        bing_client.request(methods::GET, s)
            ...
            .then([=](json::value maps_result) mutable {
                auto coordinates = maps_result[__U("resourceSets")][0][__U("resources")][0][__U("point")];
                auto lattitude = coordinates[__U("coordinates")][0].serialize();
                auto longitude = coordinates[__U("coordinates")][1].serialize();
                uri_builder ub;
                ub.append_path(lattitude + __U(",") + longitude)
            ...
            .then([=](json::value maps_result) {
                auto postal_code =
                    maps_result[__U("resourceSets")][0][__U("resources")][0][__U("address")][__U("postalCode")].as_string();
            ...
            .then([=](pplx::task<void> t) {
                ...
                {
                    message.reply(status_codes::InternalError, __U("Failed to fetch the postal code"));
                }
            });
    }
    else // get location from postal code
    {
        ...
        uri_builder ub;
        ub.append_query(__U("address"), input_text);
        ub.append_query(__U("sensor"), __U("false"));

        client.request(methods::GET, ub.to_string())
            .then([](http_response resp) { return resp.extract_json(); })
            .then([=](json::value jval) {
                auto locationstr = jval[__U("results")][0][__U("address_components")][1][__U("long_name")].as_string();
            ...
            .then([=](pplx::task<void> t) {
                ...
                {
                    message.reply(status_codes::InternalError, __U("Failed to fetch the location from postal code"));
                }
            });
    }
    return;
