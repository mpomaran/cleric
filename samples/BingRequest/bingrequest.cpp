...
#endif
        if (env_http_proxy_string == __U("auto"))
    ...
    file_buffer<uint8_t>::open(outputFileName, std::ios::out)
        .then([=](streambuf<uint8_t> outFile) -> pplx::task<http_response> {
            *fileBuffer = outFile;
            ...
            http_client client(__U("http://www.bing.com/"), client_config_for_proxy());
            return client.request(methods::GET, uri_builder(__U("/search")).append_query(__U("q"), searchTerm).to_string());
        })
