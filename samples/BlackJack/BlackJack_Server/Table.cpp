...
void DealerTable::DoubleDown(http_request message)
{
    ...
    {
        message.reply(status_codes::Forbidden, __U("Not your turn"));
        return;
    }
    ...
    {
        message.reply(status_codes::Forbidden, __U("Not enough money"));
        return;
    }
    if (current.Hand.cards.size() > 2)
    {
        message.reply(status_codes::Forbidden, __U("Too many cards"));
        return;
    }
...
void DealerTable::Wait(http_request message)
{
    ...
    if (itr == query.end())
    {
        message.reply(status_codes::Forbidden, __U("name and amount are required in query"));
        return;
    }
    else
        name = itr->second;
...
void DealerTable::Insure(http_request message)
{
    ...
    if (playerIdx > 0)
    {
        ...
        {
            message.reply(status_codes::Forbidden, __U("Already insured"));
            return;
        }

        if (dealer.cards.size() < 1 || dealer.revealBoth || dealer.cards[0].value != CV_Ace)
        {
            message.reply(status_codes::Forbidden, __U("Dealer is not showing an Ace"));
            return;
        }
