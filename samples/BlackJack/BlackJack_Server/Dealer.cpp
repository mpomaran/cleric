...
void BlackJackDealer::handle_get(http_request message)
{
    ...
    if (paths.empty())
    {
        message.reply(status_codes::OK, TablesAsJSON(__U("Available Tables"), s_tables));
        return;
    }
...
void BlackJackDealer::handle_post(http_request message)
{
    ...
    if (table->Players.size() < table->Capacity)
    {
        ...
        {
            message.reply(status_codes::Forbidden, __U("Player name is required in query"));
        }
    }
    else
    {
        utility::ostringstream_t os;
        os << __U("Table ") << table->Id << __U(" is full");
...
void BlackJackDealer::handle_delete(http_request message)
{
    ...
    if (paths.empty())
    {
        message.reply(status_codes::Forbidden, __U("TableId is required."));
        return;
    }
    utility::string_t wtable_id = paths[0];
...
void BlackJackDealer::handle_put(http_request message)
{
    ...
    {
        message.reply(status_codes::Forbidden, __U("TableId and request are required."));
    }
    utility::string_t wtable_id = paths[0];
    ...
    {
        message.reply(status_codes::Forbidden, __U("Unrecognized request"));
    }
};
